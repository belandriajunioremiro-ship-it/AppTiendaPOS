    ...
    <aside className="h-full flex flex-col">
      <div className="px-5 pt-5 pb-3 border-b border-border">
        <div className="flex flex-col items-center gap-0.5">
          ...
          <span className="font-display text-base font-bold text-foreground leading-tight mt-1">
            Tienda<span className="text-amber">POS</span>
          </span>
          {storeName && (
            <p className="text-xs font-semibold text-foreground leading-tight">{storeName}</p>
          )}
        </div>
      </div>
          ...
          return (
            <a
              ...
              className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all duration-200 ${
                isActive
                  ? 'bg-amber/[0.08] text-primary font-medium'
      ...
      <div className="px-4 py-3 border-t border-border">
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className="p-1 rounded-lg text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all shrink-0"
            title="Cerrar Sesión"
          >
        ...
        {userRole && (
          <div className="mt-2 pl-[38px]">
            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-semibold uppercase tracking-wider border ${roleColors[userRole] || 'text-muted-foreground border-border bg-muted/50'}`}>
              {userRole}
            </span>
          </div>
        )}
